...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!checkScope(auth, "mileage:read")) return apiError("Insufficient scope", 403);
    ...
    // Mileage is linked via trips, so we need to join
    const query = supabase
      .from("mileage")
      .select("*, trips!inner(organization_id)", { count: "exact" })
      .eq("trips.organization_id", auth.organizationId)
