...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!checkScope(auth, "export:sap")) return apiError("Insufficient scope", 403);
    ...
    if (!erpConfig) return apiError("SAP integration not configured", 400);
    ...
    const idoc = generateSapIDoc(exportData);

    return new Response(idoc, {
      headers: {
